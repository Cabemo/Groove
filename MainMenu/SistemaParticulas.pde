...
class SistemaParticulas {
    ...
    void generarPosiciones(int size) {
        ...
        for (int i = 0; i < posiciones.length; i++) {
            for (int j = 0; j < 2; j++) {
                ...
                } else if (j == 1) posiciones[i][j] = random(height * 4 / 7) + floor((height / 8));
            }
        }
    }
    void seleccionados() {
        ...
        for (int i = 1; i < seleccionados.length; i++) {
            ...
            if (jugador.x >= posiciones[i][0] - (displayWidth / 40) && jugador.x <= posiciones[i][0] + (displayWidth / 40)) {
                if (jugador.y >= posiciones[i][1] - (displayWidth / 40) && jugador.y <= posiciones[i][1] + (displayWidth / 40)) {
                    tiempo[i] += frameRate;
                    fill(255);
                    ellipse(100, 100, map(tiempo[i], 0, limSuperior, 0, 150), map(tiempo[i], 0, limSuperior, 0, 150));
                    if (tiempo[i] >= limInferior && tiempo[i] < limSuperior) {
                        players[i - 1].play();
    ...
    void centroG(float mainX, float mainY) {
        ...
        for (int i = 0; i < sp.length; i++) {
            for (Ball b: sp[i]) {
                if (i == 0) {
                    b.centroG(mainX, mainY, false, fuerzas[i]);
                } else {
                    if (seleccionados[i]) {
                        fill(255);
                        rect(x, height - width / 40, width, height / 12);
                        b.centroG(mainX, mainY, false, fuerzas[i]);
                    } else b.centroG(posiciones[i][0], posiciones[i][1], true, fuerzas[i]);
                }
            }
        }
    }
    void dance(int indice) {
        boolean status = false;
        float temp = 0;
        for (int j = 0; j < beatDetectors[indice - 1].specSize(); j++) temp += beatDetectors[indice - 1].getBand(j);
        if (players[indice - 1].isPlaying()) {
            temp /= beatDetectors[indice - 1].specSize();
            fuerzas[indice] = temp;
            status = true;
        } else {
            status = false;
