...
atras = new Boton(displayWidth - 200, 100, 90, 40, "BACK", 15, baron);
play = new Boton(displayWidth - 300, 100, 90, 40, "PLAY", 15, baron);
...
void mouseClick(){
    float tamano = map(tiempo, 0, 700, 0, 75);
    stroke(255);
    noFill();
    strokeWeight(4);
    ...
    if(tiempo >= 700) {
      ...
      for(int i = 1; i < p1.seleccionados.length; i++) {
        if(p1.seleccionados[i]) players[i - 1].loop(1);
    }
    }
    }
